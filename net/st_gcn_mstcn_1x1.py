...
'''
st_gcn_mstcn,只改ms-tcn，没加注意力，按比例分,
'''
class Model(nn.Module):
    ...
    def __init__(self, in_channels, num_class, graph_args,
        ...
        self.mstcn_importance = nn.ParameterList([
            nn.Parameter(torch.ones(3))  # initial with one
            for i in self.st_gcn_networks
        ])
    ...
    def forward(self, x):
        ...
        # forward
        for gcn, importance, mstcn_importance in zip(self.st_gcn_networks, self.edge_importance, self.mstcn_importance):  # edge_importance与st-gcn层一一对应,
            # gcn实际上代表的是st-gcn的一层
            x, _ = gcn(x, self.A * importance, mstcn_importance)  # 注意在forward传入的A并不是单纯的self.A,而是self.A *
            # importance
...
class st_gcn(nn.Module):
    ...
    def __init__(self,
        ...
        # )
        self.tcn_start = nn.Sequential(  # 一个有序的容器，神经网络模块将按照在传入构造器的顺序依次被添加到计算图中执行
            nn.BatchNorm2d(out_channels),  # 在卷积神经网络的卷积层之后总会添加BatchNorm2d进行数据的归一化处理，这使得数据在进行Relu之前不会因为数据过大而导致网络性能的不稳定
            nn.ReLU(inplace=True),  # inplace=True->在通过relu()计算时的得到的新值不会占用新的空间而是直接覆盖原来的值，为False则相反
        )
        self.conv_1x1_start = nn.Conv2d(  # TCN，用的二维卷积，####这是一个1D卷积，卷积的维度是时间维度，他的卷积核一行多列
            out_channels,
            int(out_channels ** 0.5),
            (1, 1),  # 卷积核在第一位度大小为kernel_size[0]：卷积核的第一维即 9 帧，第二维度大小为1
            (1, 1),  # 步长
            (0, 0)  # 填充
        )
        self.tcn_1 = nn.Conv2d(  # TCN，用的二维卷积，####这是一个1D卷积，卷积的维度是时间维度，他的卷积核一行多列
                int(out_channels ** 0.5),
                int(out_channels ** 0.5),
                (3, 1),  # 卷积核在第一位度大小为kernel_size[0]：卷积核的第一维即 9 帧，第二维度大小为1
                (stride, 1),  # 步长
                (1, 0)  # 填充
            )
        self.tcn_2 = nn.Conv2d(  # TCN，用的二维卷积，####这是一个1D卷积，卷积的维度是时间维度，他的卷积核一行多列
            int(out_channels ** 0.5),
            int(out_channels ** 0.5),
        ...
        self.tcn_3 = nn.Conv2d(  # TCN，用的二维卷积，####这是一个1D卷积，卷积的维度是时间维度，他的卷积核一行多列
            int(out_channels ** 0.5),
            int(out_channels ** 0.5),
            (15, 1),  # 卷积核在第一位度大小为kernel_size[0]：卷积核的第一维即 9 帧，第二维度大小为1
            (stride, 1),  # 步长
            (7, 0)  # 填充
        )
        self.conv_1x1_end = nn.Conv2d(  # TCN，用的二维卷积，####这是一个1D卷积，卷积的维度是时间维度，他的卷积核一行多列
            int(out_channels ** 0.5),
            out_channels,
            (1, 1),  # 卷积核在第一位度大小为kernel_size[0]：卷积核的第一维即 9 帧，第二维度大小为1
            (1, 1),  # 步长
        ...
        self.tcn_end = nn.Sequential(  # 一个有序的容器，神经网络模块将按照在传入构造器的顺序依次被添加到计算图中执行
            nn.BatchNorm2d(out_channels),
            nn.Dropout(dropout, inplace=True),
        )
    ...
    def forward(self, x, A, mstcn_importance):

        res = self.residual(x)
        x, A = self.gcn(x, A)
        # x = self.tcn(x, mstcn_importance) + res
        # x = self.tcn(x) + res
        x = self.tcn_start(x)
        x = self.conv_1x1_start(x)
        x1 = self.tcn_1(x) * mstcn_importance[0]
        x2 = self.tcn_2(x) * mstcn_importance[1]
        x3 = self.tcn_3(x) * mstcn_importance[2]
        x = x1 + x2 + x3
        x = self.conv_1x1_end(x)
        x = self.tcn_end(x)
        x = x + res
