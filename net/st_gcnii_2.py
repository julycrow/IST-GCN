...
from net.utils.tgcnii_2 import ConvTemporalGraphical
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, (gcn, importance) in enumerate(
            ...
            # gcn实际上代表的是st-gcn的一层
            x, _, h0 = gcn(x, self.A * importance, h0, i + 1)  # 注意在forward传入的A并不是单纯的self.A,而是self.A * importance
...
class st_gcn(nn.Module):
    ...
    def forward(self, x, A, h0, i):
        res = self.residual(x)
        x, A, h0 = self.gcn(x, A, h0, i)
        x = self.tcn(x) + res
        h0 = self.down(h0)
        return self.relu(x), A, h0
