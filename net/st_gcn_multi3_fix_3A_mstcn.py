...
from net.utils.tgcn_multi3_fix_3A import ConvTemporalGraphical
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # forward
        for gcn, importance, importance2, importance3, mstcn_importance in zip(self.st_gcn_networks,
                                                                               self.edge_importance,
                                                                               self.edge_importance2,
                                                                               self.edge_importance3,
                                                                               self.mstcn_importance):  # edge_importance与st-gcn层一一对应,
            # gcn实际上代表的是st-gcn的一层
            x, _ = gcn(x, self.A, importance, importance2, importance3, mstcn_importance)  # 注意在forward传入的A并不是单纯的self.A,而是self.A *
            # importance
        # global pooling
...
class st_gcn(nn.Module):
    ...
    def forward(self, x, A, importance, importance2, importance3, mstcn_importance):

        res = self.residual(x)
        x, A = self.gcn(x, A, importance, importance2, importance3)
